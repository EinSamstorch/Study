...
public class Main2 {
     ...
     */
    private static ConcurrentHashMap<String, TreeSet<Order>> sales = new ConcurrentHashMap<>();
    ...
    public static void write(String itemId, String userId) {
        TreeSet<Order> notifies = sales.computeIfAbsent(itemId, k -> new TreeSet<>());
        notifies.add(new Order(new Date(), userId, itemId));
    }
    ...
    public static List<Order> read(String itemId) {
        ...
        TreeSet<Order> orders = sales.get(itemId);
        // 如果为空，说明该商品现在没有订单记录
        if (orders == null) {
            return res;
        }
        // data应该也是线程不安全的，应该用localdate
